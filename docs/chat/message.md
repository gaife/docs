...
```bash
curl 'https://developer.gaife.com/api/messages' \
...
**Curl Example**:
```bash
curl -X POST 'https://developer.gaife.com/api/media/presigned-url' \
  -H 'Authorization: Bearer {{API_KEY}}' \
  -H 'Content-Type: application/json' \
  -d '{
    "file_name": "report.pdf",
...
**Curl Example**:
```bash
curl --location 'https://developer.gaife.com/api/messages' \
